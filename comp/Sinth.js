...
let play = false;
function PlayFull(aContext, startTime, sound, tempo, notes, notifyStop) {
    const spb = 60.0 / tempo;
    let nIndex = 0;
    notes.forEach((n, i) => scheduleNote(aContext, startTime + (notes[i].Beat - 1) * spb, sound, n, spb));
    notifyStop();
}
function PlaySequence(aContext, startTime, sound, tempo, notifyStop) {
